...
const FINAL_PROMPT =
  "You receive the user input and the AI agent response with a solution to the inquiry. Formulate the final response to the user based on the answer provided by the agent. Respond with the final answer only.";
...
export async function POST(req: Request) {
  ...
  if (!classificationResult) {
    return streamText({
      model: MODEL,
      prompt:
        "You were not able to determine the user intent accurately. Respond with a request to clarify.",
...
const classifyMessages = async (
  ...
  const classifyResponse = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/classification`,
    {
      method: "POST",
...
const callAgent = async (messages: CoreMessage[], route: string) => {
  const agentResponse = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/agent/${route}`,
    {
      method: "POST",
      body: JSON.stringify({ messages }),
      headers: {
...
const agentRouter = async (
  ...
  switch (classificationResult) {
    ...
    case "erc20_transfer_to_ens":
      return callAgent(messages, "erc20_transfer");
    case "erc20_swap":
      return callAgent(messages, "uniswap_swap");
    case "restake":
    ...
    default:
      return {
        answer:
          "I am sorry, I do not understand your request. Please provide clarification.",
      };
  }
};
